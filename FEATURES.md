...
| Outreach: who to message for the top-ranked new roles | `tests/outreach.mjs` | yes | Every link is a LinkedIn SEARCH built from the employer name on the row. Nothing in the database knows who the hiring manager is, so a name here would be fabricated research dressed up as a lead, which is the one thing this page must never do. The test refuses any `linkedin.com/in/` href, requires four searches per card, and requires each one to carry a quoted employer (an unscoped search returns the whole of LinkedIn). Also: ranks descending, an unpublished band says so rather than reading "competitive", each message shape ends in a question, and the page is light with the OS set to dark. Known-bad: a temp mutation making step 1 emit `linkedin.com/in/some-person` is required to fail, and does -- the first attempt at that proof reported a pass because the mutation never reached the served build |
