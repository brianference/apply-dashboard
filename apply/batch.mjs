...
import { sameJob } from '../ingest/match.mjs';
  ...
  if (submit) {
    try {
      ...
      /* Aggressive: an exact key or URL match is not enough. The same job is
         ...
         a duplicate application. sameJob() compares on a NORMALISED company
         and title, shared with the ingest-time guard in sync-to-d1.mjs so the
         two checks cannot drift apart. Brian's rule, 2026-08-25: aggressively
         prevent duplicates. */
      const clash = fresh.find(f => isDone(f)
        && (f.url === j.url || f.dedupe_key === j.dedupe_key || sameJob(f, j)));
      if (clash) {
